...
[<NT_53/>](https://smartdatamodels.org)  
...
- `motorDirection[string]`: Enum:'-1','1'. Define la orientación física del montaje del motor, que determina la polaridad de la rotación (hacia el este = 1; hacia el oeste = -1)  . Modelo: [https://schema.org/Text](https://schema.org/Text) 
...
- `operatingMode[string]`: Enum:'automático','manual','safesnow','safesnowsensor','safewind','safewindsensor','mantenimiento'. Define el estado funcional actual del panel. Este modo dicta si el seguidor sigue el algoritmo solar o responde a eventos/sobrecargas externos  . Modelo: [https://schema.org/Text](https://schema.org/Text) 
...
- `restingAngle[number]`: La posición angular específica que asume el panel durante la noche o cuando el sol está debajo del horizonte. Esta posición se utiliza para estacionar el seguidor hasta el siguiente amanecer  . Modelo: [https://schema.org/Number](https://schema.org/Number) 
...
- `windSafetyAngle[number]`: El punto de referencia de seguridad que asume el panel cuando las velocidades del viento superan el umbral operativo. Esta posición minimiza el efecto de vela y reduce el estrés estructural causado por la elevación o presión del viento  . Modelo: [https://schema.org/Number](https://schema.org/Number) 
<!-- /30-PropertiesList -->  
...
![Smart Data Models](https://smartdatamodels.org/wp-content/uploads/2022/01/SmartDataModels_logo.png "Logo") 